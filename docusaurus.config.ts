...
// Docs flagged `deprecated: true` in frontmatter stay reachable by URL but
// are hidden from the autogenerated sidebar (legacy command names).
const DOC_ROOT = path.join(__dirname, 'docs');

function collectDeprecatedIds(dir: string): Set<string> {
  ...
  const walk = (d: string) => {
    for (const entry of fs.readdirSync(d, {withFileTypes: true})) {
      ...
      if (entry.isDirectory()) {
        ...
        if (m && /^deprecated:\s*true$/m.test(m[1])) {
          const rel = path.relative(DOC_ROOT, full).replace(/\\/g, '/').replace(/\.md$/, '');
          ids.add(rel);
        }
      }
    }
