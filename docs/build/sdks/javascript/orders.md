...
Orders are Mintlayer's on-chain primitive for decentralized trading: one party locks an amount (`give`) and asks for another asset (`ask`) in return. Anyone can fill an order; the creator can conclude it to reclaim the balance. See [Trading with Orders (SDK)](../../../guides/javascript/trading-with-orders.md) for a walkthrough, and the [wallet-cli orders guide](../../../guides/cli/trading-with-orders.md) for the concepts.
