...
Orders are Mintlayer's on-chain primitive for decentralized trading: one party locks an amount (`give`) and asks for another asset (`ask`) in return. Anyone can fill an order; the creator can conclude it to reclaim the balance. See [Trading with orders](../../../guides/trading-with-orders.md) for the concepts and a wallet-CLI walkthrough.
