...
description: "Script-based smart contracts, ACL, DID, and programmable pools."
---
...
The Ethereum whitepaper [ \($$^1‎^0$$\)](/whitepaper/4-decentralized-finance-defi#footnotes) states that “Turing-incompleteness is not even that big of a limitation”. Out of all the contract examples that the Ethereum team had conceived when writing a whitepaper, only one required a loop, and even that loop could be removed by making 26 repetitions of a one-line piece of code. A more recent study[ \($$^1‎^1$$\)](/whitepaper/4-decentralized-finance-defi#footnotes) says that only 6,9% of smart contracts built on the Ethereum Virtual Machine truly require Turing-complete language functions. It also notes that the vast majority of Ethereum’s smart contracts can be coded to function the same way on a Turing-incomplete machine.
...
Mintlayer supports Lightning Network for all MLS-01 tokens and other HTLC script-based contracts used explicitly for the DEX, like atomic swap \([§5.2.](/whitepaper/5-decentralized-exchange-dex#5-2-the-atomic-swap-dex)\) or lightning swaps \([§5.7.](/whitepaper/5-decentralized-exchange-dex#5-7-lightning-network-dex)\), which are also interoperable with the Bitcoin blockchain.
...
The controller can be the token issuer, a service, an identity provider, or more generically, an oracle. The standard identification procedure required to ask the controller for the countersignature can be performed using the DID \([§4.4.](/whitepaper/4-decentralized-finance-defi#4-4-decentralized-identity-did-and-oracles)\).
